...
run (message)
{
  try
  {
      ...
      embed
        //.setColor(`${color}`)
        .setTitle(`Statbot v${process.env.VERSION}`)
        .setThumbnail(`${this.client.botAvatar}`)
        .setFooter(`Brought to you by ${this.client.owners[0].tag}.`)
        .setDescription(`Statbot: A Discord bot that fetches data from Minecraft Speedrunners' stat sheets\n\n**Prefix: ${prefix}**`)
        ;
